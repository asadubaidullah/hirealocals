...
def is_test_or_dummy_email(to_email: str | None) -> bool:
    """Return True if the email belongs to a test/dummy domain or synthetic test account pattern."""
    if not to_email:
        return True
    ...
    # Synthetic test fixtures generated by E2E test suites (e.g. local_p6_..., traveler_p5_...)
    if re.match(r"^(local|traveler|admin|tourist|user|other)_[a-z0-9]+_\d+", local_part):
        return True
    if re.match(r"^(local|traveler|admin|tourist|user|other)_p\d+_", local_part):
        return True
...
def _deliver_outbox_row(session: Session, row: EmailOutbox) -> None:
    if is_test_or_dummy_email(row.to_email):
        row.status = "test_suppressed"
        row.last_error = "Suppressed: test/dummy recipient address blocked from production SMTP."
        session.add(row)
        session.commit()
        return
