...
export default function StandaloneReceiptPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Fallback for public preview or unauthenticated receipt verification
      setBooking({
        id: id || 1,
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        window.open(`/api/traveler/bookings/${id}/receipt`, '_blank');
        return;
      }
  ...
  const subtotal = Number(b.subtotal || 89.0);
  const fee = Number(b.platform_fee || 10.68);
